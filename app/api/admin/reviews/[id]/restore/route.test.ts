...
const restoreStandaloneReview = vi.hoisted(() => vi.fn());
const revalidateProgram = vi.hoisted(() => vi.fn(async () => {}));

vi.mock("@/lib/roles", () => ({ requireRole }));
vi.mock("@/lib/reviews", () => ({ restoreStandaloneReview }));
vi.mock("@/lib/revalidate", () => ({ revalidateProgram }));

const { POST } = await import("./route");

function post(): Request {
  return new Request("http://localhost/api/admin/reviews/review_1/restore", { method: "POST" });
}
...
beforeEach(() => {
  ...
  restoreStandaloneReview.mockReset();
...
describe("POST /api/admin/reviews/[id]/restore", () => {
  it("rejects a non-moderator and never writes", async () => {
    ...
    expect(restoreStandaloneReview).not.toHaveBeenCalled();
  });
  ...
  it("moderator: restores and revalidates the program page", async () => {
    ...
    restoreStandaloneReview.mockResolvedValue({ ok: true, programId: "prog_1" });
    const res = await POST(post(), params("review_1"));
    expect(res.status).toBe(200);
    expect(restoreStandaloneReview).toHaveBeenCalledWith("review_1", "mod_1");
    expect(revalidateProgram).toHaveBeenCalledWith("prog_1");
  });

  it("gate failure → 400, no revalidate", async () => {
    requireRole.mockResolvedValue({ ok: true, status: 200, userId: "mod_1" });
    restoreStandaloneReview.mockResolvedValue({ ok: false, reason: "Only an archived review can be restored" });
    const res = await POST(post(), params());
